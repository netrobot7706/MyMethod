...
本文档定义 **Lyre 四推杆 MIDI 控制器**的产品特有命令协议。它继承《Veloce 基础命令协议 v1.4》的所有规定（帧格式、状态机、心跳、握手、恢复出厂、错误应答），并在此基础上定义推杆配置、ADC 采集、校准等业务命令。
...
| write_pot_config_ack | 设备→上位机 | `[cmd=write_pot_config_ack;<回传字段>]` | 保存成功确认，回传上位机发送的字段及设备实际写入的值。 |
...
- `min` / `max`：
  ...
  - 若只更新了 `min` 或 `max` 之一，设备应与未更新的另一侧组合后校验 `min < max` 仍成立。**组合校验的基准是当前 RAM 中的生效值**（已经过上电合法性修正的配置），而非直接读取 Flash 中的原始值。校验通过后，将合法的新配置整体写入永久存储。
...
- **上位机建议**：尽可能发送全量配置（所有 4 个推杆的全部字段），减少 MCU 组合校验开销，并确保配置一致性。
...
设备 → 上位机: [cmd=write_pot_config_ack;0_midi_ch=1;0_cc=70;0_min=95;0_max=4010;1_midi_ch=1;1_cc=71;1_min=100;1_max=4000;2_midi_ch=1;2_cc=72;2_min=100;2_max=4000;3_midi_ch=1;3_cc=73;3_min=100;3_max=4000]
```

**示例（部分写入）**：
```
上位机 → 设备: [cmd=write_pot_config;2_cc=72]
设备 → 上位机: [cmd=write_pot_config_ack;2_cc=72]
```
...
**流模式限制**：流模式激活期间，上位机不得发送 `read_adc`，因为设备同样以 `report_adc` 应答，上位机无法区分其属于流数据还是单次应答。若设备在流模式下收到 `read_adc`，应返回错误 `[cmd=error;code=3;msg=stream_active]`。上位机应在流停止后再进行单次读取。
...
| 命令 | 方向 | 格式 | 说明 |
| :--- | :--- | :--- | :--- |
| stream_adc | 上位机→设备 | `[cmd=stream_adc;pot=<推杆>;enable=1]` | 开启指定推杆的 ADC 连续上报。可附带可选参数 `interval_ms`。 |
...
**流式上报行为**：
- 设备收到开启指令且参数合法后，先发送 `stream_adc_ack;enable=1` 确认，随后以默认 50ms 间隔（约 20Hz）的频率主动向主机发送 `report_adc` 帧。
- 同一时刻仅允许一个推杆处于流模式。若开启 `pot=1` 时 `pot=0` 正在流模式，设备应先自动关闭 `pot=0` 的流，然后开启 `pot=1` 的流。
- **流模式期间禁止 `read_adc`**，上位机不得在流模式下发送 `read_adc`（见 §3.3）。

**推杆切换 ACK 时序**（简化）：
当开启新推杆的流模式导致旧推杆自动关闭时，设备**仅发送新流开启的 ACK**：
```
设备 → 上位机: [cmd=stream_adc_ack;pot=1;enable=1]
```
旧流的关闭是隐式的——上位机已知"同一时刻仅允许一个推杆处于流模式"，收到 `pot=1` 的开启 ACK 即意味着 `pot=0` 已自动关闭。此设计避免了多条 ACK 与 `report_adc` 数据在上位机接收缓冲区中的时序交织。
...
**连接超时**：流模式激活期间，连接超时检测**照常进行**。上位机应继续每 10 秒发送 `ping`，设备收到后刷新超时计时器。若上位机崩溃/断连，设备在 30 秒后自动关闭流模式并回到 IDLE。

**可选参数 `interval_ms`**：
上位机可在开启流模式时指定上报间隔：
```
[cmd=stream_adc;pot=0;enable=1;interval_ms=50]
```
- 若未指定，默认 50ms（约 20Hz）。
- 取值范围：20ms ~ 500ms。设备应校验并在越界时返回 error (code=3)。
- **参数作用域**：`interval_ms` 仅对当前开启的流生效。若切换推杆时未重新指定此参数，频率**强制复位为默认值 50ms**，不继承旧流的设置。
- 此参数为上层根据不同连接介质动态调整带宽占用而设计。
...
| set_calibration_ack | 设备→上位机 | `[cmd=set_calibration_ack;pot=<推杆>;min=<值>;max=<值>]` | 校准值成功应用，回传确认值。 |
...
设备 → 上位机: [cmd=set_calibration_ack;pot=0;min=95;max=4010]
上位机 → 设备: [cmd=write_pot_config;0_min=95;0_max=4010]
设备 → 上位机: [cmd=write_pot_config_ack;0_min=95;0_max=4010]
```
...
| read_adc | 500ms | 2 | 流模式下禁止发送，否则返回错误 |
...
设备 → 上位机: [cmd=handshake_ack;id=LYRE-A1B2;ver=1.0;base_proto_ver=1.4;biz_proto_ver=1.3;max_frame_len=512]
上位机 → 设备: [cmd=read_pot_config]
设备 → 上位机: [cmd=read_pot_config;0_midi_ch=1;0_cc=70;0_min=100;0_max=4000;1_midi_ch=1;1_cc=71;1_min=100;1_max=4000;2_midi_ch=1;2_cc=72;2_min=100;2_max=4000;3_midi_ch=1;3_cc=73;3_min=100;3_max=4000]
...
设备 → 上位机: [cmd=handshake_ack;id=LYRE-A1B2;ver=1.0;base_proto_ver=1.4;biz_proto_ver=1.3;max_frame_len=512]
上位机 → 设备: [cmd=write_pot_config;0_cc=72]
设备 → 上位机: [cmd=write_pot_config_ack;0_cc=72]
```

### 6.4. 流模式切换推杆（简化 ACK 时序 + 频率复位）
```
上位机 → 设备: [cmd=stream_adc;pot=0;enable=1;interval_ms=20]
设备 → 上位机: [cmd=stream_adc_ack;pot=0;enable=1]
设备 → 上位机: [cmd=report_adc;pot=0;raw=110]
... (用户决定换一个推杆校准，未指定 interval_ms)
上位机 → 设备: [cmd=stream_adc;pot=1;enable=1]
设备 → 上位机: [cmd=stream_adc_ack;pot=1;enable=1]   ← 仅新流 ACK，频率复位为默认 50ms
...
(上位机崩溃或关闭页面)
... 设备继续发送 report_adc ...
... 30 秒内未收到任何上位机报文（包括 ping） ...
设备内部：关闭流模式 → 状态回 IDLE
(上位机重新打开页面并握手后，一切恢复正常)
```

### 6.6. 流模式下拒绝 read_adc
```
上位机 → 设备: [cmd=read_adc;pot=0]
设备 → 上位机: [cmd=error;code=3;msg=stream_active]
```
...
- 实现 `read_pot_config`、`write_pot_config`、`read_adc`、`stream_adc`、`set_calibration` 五个命令的处理函数，注册到基础协议的命令分发表（配置域）。
- `write_pot_config` 校验逻辑完整，所有非法值返回 error，不做静默裁剪。部分更新时使用 **RAM 生效值**组合校验，校验通过后全量写入 Flash。
- 上电加载配置时必须校验 `min < max`，非法则对该推杆使用出厂默认校准值（但保留 CC 和通道）。
- **流模式管理**：维护当前流推杆编号。切换推杆时，**必须先彻底关闭旧流**（包括等待硬件缓冲区中最后一条 `report_adc` 发送完成），再开启新流，并**仅发送新流的 `stream_adc_ack`**。每次显式或隐式开启新流时，若未指定 `interval_ms`，强制复位为默认值 50ms。
- 流模式期间继续响应 `ping` 并刷新超时计时器；拒绝 `read_adc` 命令（返回 code=3 `stream_active`）。
- 实现 `interval_ms` 可选参数，接收后校验范围（20~500ms）。

### 7.2. 上位机实现要点
- 在基础协议实现之上，增加产品命令的构造与解析。
- 握手后记录 `max_frame_len`，发送帧时确保数据部分不超过该值（本产品为 512）。
- 发送 `write_pot_config` 后逐字段核对回传值与发送值是否一致，不一致时提示用户。
- 尽量发送全量配置以减少 MCU 组合校验开销。
- 流模式期间正确处理 `stream_adc_ack` 和 `report_adc`。收到新推杆的 `stream_adc_ack` 即视为旧推杆流已自动关闭。
- 校准向导中使用流模式替代单次 `read_adc` 轮询，提升体验。流模式期间禁止发送 `read_adc`。
- 流模式期间继续每 10 秒发送 `ping`，保持连接活跃。
...
改动集中在删除所有 `_ack` 帧中的 `status=ok`，以及基础协议 §4.1 的措辞调整，其余内容与上一版一致。可以直接作为开发基线使用。

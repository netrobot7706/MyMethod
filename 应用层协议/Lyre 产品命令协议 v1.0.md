...
本文档定义 **Lyre 四推杆 MIDI 控制器**的产品特有命令协议。它继承《Lyre 基础命令协议 v1.0》的所有规定（帧格式、状态机、握手、恢复出厂、错误应答），并在此基础上定义推杆配置、ADC 采集、校准等业务命令。

### 1.1. 产品帧长
本产品最大帧数据长度继承基础协议默认值 **512 字节**。设备在握手时报告 `max_frame_len=512`。

### 1.2. 引用基础协议
以下章节直接引用《Lyre 基础命令协议 v1.0》，本文档不再重复：
...
**文档版本**：1.0
**最后更新**：2026-07-15
**适用范围**：Lyre 四推杆 MIDI 控制器（须配合《Lyre 基础命令协议 v1.0》使用）

...
| 文档版本 | v3.2 (量产加固版) |
| 日期 | 2026-07-10 |
| v3.1→v3.2 变更 | ① **Flash 写入安全喂狗**：`flash_store_write_file()` 内部块操作后调用 `watchdog_update()`，通过 `CFG_WDT_SAFE_WRITE` 宏控制，防止长擦除触发 WDT 复位。② **ADC 过采样语义统一**：明确 `adc_sampler_read(pin, 16)` 内部实现为"读 17 次丢首值，后 16 次平均"，消除文档与代码歧义。③ **信号链初始化补全**：`signal_conditioner_init()` 增加 `memset` 清零 + `filter_window_size` 默认值 4，消除上电瞬态跳变。④ **上位机协议约束**：KAL 及指令表中明确要求 `save_config` 后上位机等待 ≥1000ms 并重连，零固件成本规避 USB 断连风险。 |
...
| 组件 | 规格 |
| :--- | :--- |
| 主控 | RP2040-Zero (双核 Cortex-M0+, 264KB RAM, 2MB Flash) **注：v3.2 仅使用单核** |
| 推杆 ×4 | 100mm 行程线性电位器 |
...
| 看门狗 | **硬件 WDT (2000ms)** | `setup()` 中 `watchdog_enable(2000, 1)`，`loop()` 末尾 + **Flash 写入内部** 喂狗 |
...
| 信号采集与调理 | 自实现 5 级可开关链 | 每级独立使能+参数可调，**所有分母参数必须强钳位**，**init 必须 memset 清零** |
| 持久化存储 | LittleFS | arduino-pico core 原生支持 |
| LED 驱动 | Adafruit NeoPixel | RP2040 PIO 驱动 |
...
-   **看门狗强制。** 主循环末尾必须包含 `watchdog_update()`；🔥 **Flash 写入内部通过 `CFG_WDT_SAFE_WRITE` 宏安全喂狗**。
-   **USB 状态保持。** 🔥 **上位机发送 `save_config` 后必须等待 ≥1000ms 并准备重连**，固件侧写入前后显式 `tud_task()`。
-   **信号链初始化。** 🔥 `signal_conditioner_init()` 必须 `memset` 清零整个结构体，并将 `filter_window_size` 设为默认值 4。
...
│  │(LittleFS+WDT)│  │(TinyUSB读写封装)│  │   (NeoPixel驱动)       │ │
│  └──────────────┘  └────────────────┘  └────────────────────────┘ │
│  ┌──────────────┐  ┌────────────────┐  ┌────────────────────────┐ │
...
每 1ms 执行一轮 loop()：
  ...
  3. midi_dispatcher_poll() → 收到 JSON → cmd_handler_process()
...
| 模块 | 文件 | 职责边界 | 复用性 |
| :--- | :--- | :--- | :--- |
| adc_sampler | adc_sampler.h/c | 🔥 200Hz 触发的 17 次过采样（读 17 丢 1 平均 16） | 🟢 通用 |
| signal_conditioner | signal_conditioner.h/c | 5 级可开关信号调理链（含防除零钳位 + memset 初始化） | 🟢 通用 |
...
| config_manager | config_manager.h/c | 通用二进制块（Blob）存储代理：Magic 校验 + Flash 搬运 | 🟢 通用 |
| ws2812_hal | ws2812_hal.h/c | Adafruit NeoPixel 初始化与像素控制 | 🟢 通用 |
...
| main | main.ino | 单核 setup/loop 入口，Mediator 事件路由，WDT 喂狗 | 🔴 专属 |
 ...
 *
 * 🔥 v3.2: 当 count=16 时，内部连续读取 17 次，丢弃第 1 次，
...
### 6.3 `signal_conditioner` 规格（5 级可开关链 + 防御加固 + 初始化补全）
...
-   🔥 **`signal_conditioner_init()` 必须 `memset(sc, 0, sizeof(*sc))` 清零整个结构体，然后将 `filter_window_size` 设为默认值 4**，防止上电瞬态跳变和早期除零。
...
| 2 | 死区检测 | `deadzone_enable` | `deadzone_threshold` (ADC 原始值) | 跳过 | threshold > 2047 钳位为 2047 |
...
typedef struct {
    ...
    // Stage 2: 死区检测
    bool     deadzone_enable;
    uint16_t deadzone_threshold;
...
```c
void signal_conditioner_init(signal_conditioner_t *sc) {
    // 🔥 v3.2: 全结构体清零，消除上电瞬态跳变
    memset(sc, 0, sizeof(*sc));
    // 🔥 v3.2: 设置安全默认值，防止 filter_enable=true 时早期除零
...
int8_t signal_conditioner_process(signal_conditioner_t *sc, uint16_t raw) {
    ...
    // Stage 2: 死区检测
    if (sc->deadzone_enable) {
        uint16_t thr = sc->deadzone_threshold;
        if (thr > 2047) thr = 2047;
        if (value < thr) value = thr;
        if (value > 4095 - thr) value = 4095 - thr;
    }
    ...
    if (sc->change_threshold_enable) {
        ...
        if (thr < 0) thr = 0;
        if (thr > 127) thr = 127;
        if (abs(midi_val - sc->last_sent_midi) <= thr) {
            return -1;
        }
    }

    if (midi_val != sc->last_sent_midi) {
...
Step 2: 开启 Stage 1 滤波 → 调整 window_size，观察滤波效果
...
入站链：
```
midi_dispatcher_poll() → sysex_decoder_process() → 提取 JSON → on_json_ready() → cmd_handler_process()
```
...
| save_config | `{"cmd":"save_config",...}` | `{"cmd":"save_config_ack","status":"ok"}` | 🔥 **唯一写 Flash 的配置指令。上位机发送后必须等待 ≥1000ms 并准备重连** |
| factory_reset | `{"cmd":"factory_reset"}` | `{"cmd":"factory_reset_ack","status":"ok"}` | 恢复出厂（写 Flash），同上重连约束 |
| calibrate | `{"cmd":"calibrate","ch_num":1,"min":200,"max":3800}` | `{"cmd":"calibrate_ack","status":"ok"}` | ⚠️ 仅更新 RAM，不写 Flash |
| set_signal_stage | `{"cmd":"set_signal_stage","ch_num":1,"stage":"filter","enable":true,"param":16}` | `{"cmd":"set_signal_stage_ack","status":"ok"}` | 🔥 运行时调整信号链参数 |
...
static const device_config_t FACTORY_DEFAULT_CONFIG = {
    .pots = {
        { .channel = 1, .cc = 1, .cal_min = 100, .cal_max = 4000,
          .signal_chain = { .filter_enable = false, .filter_window_size = 8,
                            .deadzone_enable = false, .deadzone_threshold = 0,
...
### 9.2 `config_manager` 规格（简化版）

废除 CRC 校验，仅保留 Magic Number 检查。读取时若 magic 不匹配，直接恢复出厂默认值，**并通过 SysEx Log Event 上报 `"LittleFS Magic Mismatch, Factory Reset"`**。
...
bool config_manager_init(const void *default_blob, size_t blob_size, void *runtime_blob);
...
// 🔥 v3.2: 定义此宏后，write_file 内部每次块操作后自动喂狗
// 防止 LittleFS 长擦除触发 WDT 复位
...
> ⚠️ `flash_store_write_file()` 及其直接调用的底层函数**必须标记 `__not_in_flash_func()`**。这是 v3.2 中唯一需要该宏的地方。
> 🔥 **v3.2 新增**：当 `CFG_WDT_SAFE_WRITE` 定义为 1 时，`flash_store_write_file()` 内部每次 LittleFS 块擦除/写入操作后必须调用 `watchdog_update()`。
...
废除独立定时器/状态机。改为主循环内计数器驱动，每 1ms 调用一次 `led_engine_update()`。
...
### 11.2 单核 setup / loop（含 WDT + 200Hz ADC + USB 保活）
...
void setup() {
    ...
    led_engine_init();
    flash_store_init();
    config_manager_init(&FACTORY_DEFAULT_CONFIG, sizeof(device_config_t), &g_runtime_config);

    for (int i = 0; i < POT_COUNT; i++) {
        signal_conditioner_init(&g_conditioners[i]);  // 🔥 v3.2: 内部 memset + 默认值
...
void on_json_ready(const char *json, size_t len) {
    cmd_handler_process(json, len);
}

void loop() {
    static uint32_t adc_counter = 0;

    // 🔥 v3.2: 200Hz ADC 过采样 (每 5ms 触发 1 次 17x 过采样，~140µs)
    adc_counter++;
    if (adc_counter >= 5) {
        adc_counter = 0;
    ...
    // 🔥 v3.2: 喂狗（必须放在 loop 最后）
    watchdog_update();
}
```

### 11.3 指令事件处理（含 USB 保活 + Log Event）

```cpp
// 🔥 v3.2: 辅助函数，通过 SysEx 上报日志
    ...
    switch (event->type) {
        ...
        case CMD_EVENT_SAVE_CONFIG: {
            ...
            if (config_codec_deserialize(event->raw_json_ptr, event->json_len, &shadow_config)) {
                ...
                // 🔥 v3.2: Flash 写入前刷新 USB 缓冲区
                tud_task();

                // 🔥 v3.2: 内部通过 CFG_WDT_SAFE_WRITE 安全喂狗
                config_manager_save(&g_runtime_config, sizeof(device_config_t));

                // 🔥 v3.2: 写入后再次刷新，加速主机端恢复
                tud_task();

                len = snprintf(tx_buf, sizeof(tx_buf), "{\"cmd\":\"save_config_ack\",\"status\":\"ok\"}");
            } else {
                len = snprintf(tx_buf, sizeof(tx_buf), "{\"cmd\":\"save_config_ack\",\"status\":\"error\"}");
            }
        ...
        case CMD_EVENT_FACTORY_RESET:
            ...
            config_manager_factory_reset(&g_runtime_config, sizeof(device_config_t));
            for (int i = 0; i < POT_COUNT; i++) {
                signal_conditioner_set_calibration(&g_conditioners[i],
                    g_runtime_config.pots[i].cal_min, g_runtime_config.pots[i].cal_max);
            }
            len = snprintf(tx_buf, sizeof(tx_buf), "{\"cmd\":\"factory_reset_ack\",\"status\":\"ok\"}");
            break;
...
| 2 | 🔥 保存配置后 PC 端可能短暂断开 USB | 中 | 需等待自动重连 | **上位机 save_config 后等待 ≥1000ms 并自动重连** | 固件侧已 tud_task() 缓解，协议层兜底 |
| 3 | Flash 数据损坏导致配置丢失 | 接近零 | 需重新校准 | magic number 不匹配时恢复默认值 + SysEx Log 上报 | 不做 CRC，仅 magic number 检查 |
| 4 | USB 意外断连后需重新插拔 | 低 | 中断演奏数秒 | 用户自行重插 | 不做断连缓冲，TinyUSB 自动重枚举 |
...
| 模块 | 开发指令重点 | 单测策略 |
| :--- | :--- | :--- |
| adc_sampler | 🔥 17 次读/丢首值/16 次平均，200Hz 调度 | PC Mock `analogRead` + 验证首值丢弃 |
| signal_conditioner | 5 级可开关链 + 各级边界钳位 + 🔥 memset 初始化 | PC 序列验证：非法参数注入、除零测试、上电瞬态测试 |
| flash_store | LittleFS 挂载与文件读写 + 🔥 CFG_WDT_SAFE_WRITE | 烧录验证 + WDT 超时压力测试 |
| ws2812_hal | Adafruit NeoPixel 初始化 | 烧录验证 |
...
| 模块 | 依赖 | 开发指令重点 | 单测策略 |
| :--- | :--- | :--- | :--- |
| config_manager | flash_store | Blob 存储 + Magic 校验 + Log Event 触发 | PC Mock |
...
| 模块 | 依赖 | 开发指令重点 | 单测策略 |
| :--- | :--- | :--- | :--- |
| config_codec | device_config.h | ArduinoJson 双向转换 + Log Event 序列化 | PC JSON 验证 |
| cmd_handler | 无 | 纯事件派发 + `set_signal_stage` 新指令 | PC JSON 验证 |
| main.ino | 所有模块 | 单核主循环 + WDT 喂狗 + 200Hz ADC + USB 保活 | 全量集成测试 |
...
### 上下文
本项目是 Lyre USB MIDI 控制器固件（RP2040 单核架构，v3.2 量产加固版）。
...
8. 🔥 信号链相关模块：所有分母参数必须在运算前进行非零和边界强钳位；init 必须 memset 清零 + filter_window_size 默认值 4。
9. 🔥 ADC 采样模块：adc_sampler_read(pin, 16) 内部必须读取 17 次，丢弃第 1 次，后 16 次算术平均。
10. 🔥 Flash 存储模块：当 CFG_WDT_SAFE_WRITE=1 时，write_file 内部每次块操作后调用 watchdog_update()。
11. 🔥 主循环相关模块：末尾必须包含 watchdog_update()。

### 验收标准
1. [功能验收项]
2. [边界条件测试项]
3. 🔥 [防御钳位/WDT/USB保活/过采样语义/初始化 专项测试项]
```
...
| **v3.1→v3.2** | **ADC 过采样次数描述与调用不一致** | **明确 read(pin,16) = 读17丢1平均16，更新注释** | **adc_sampler** |
...
**文档版本 v3.2 量产加固版 — 2026-07-10**
经过 10 轮架构审计迭代。v3.2 在 v3.1 基础上补全了 Flash 写入安全喂狗、ADC 过采样语义统一、信号链初始化安全、上位机协议层重连约束四项生产级补丁。本文档为自包含完整规格书，可直接交付 AI 程序员进行逐模块代码生成。

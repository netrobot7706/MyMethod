...
| 文档版本 | v3.1 (量产加固版) |
| 日期 | 2026-07-10 |
| v3.0→v3.1 变更 | ① **ADC 采样重构**：废除每 1ms 阻塞式过采样，改为 200Hz 触发频率（每 5ms 执行一次 17×4 过采样），单次耗时 ~140µs，占 5ms 时间片约 2.8%，兼顾噪声抑制与主循环确定性。② **信号链防御加固**：`signal_conditioner_process` 增加 `filter_window_size` 非零与边界强钳位，防止除零死机或内存越界。③ **USB 保活机制**：Flash 写入期间显式调用 `tud_task()` 或上位机协议层增加重连容忍，防止主机端超时剔除设备。④ **看门狗兜底**：新增硬件 Watchdog（2000ms），`setup()` 开启，`loop()` 末尾喂狗。⑤ **量产可观测性**：新增 SysEx Log Event 上报机制，量产模式下异常状态通过 JSON over SysEx 主动上报，替代 CDC 串口。 |
...
| 组件 | 规格 |
| :--- | :--- |
| 主控 | RP2040-Zero (双核 Cortex-M0+, 264KB RAM, 2MB Flash) **注：v3.1 仅使用单核** |
| 推杆 ×4 | 100mm 行程线性电位器 |
...
| ADC 采样率 | **200Hz (5ms/次)** | 主循环 1ms 计数器驱动，每 5 次循环采样 1 次，单次读取不阻塞 |
| Flash 写入 | 主循环内同步执行 | 写入期间主循环暂停 ~400ms，**必须配合 USB 保活或上位机重连机制** |
| 看门狗 | **硬件 WDT (2000ms)** | `setup()` 中 `watchdog_enable(2000, 1)`，`loop()` 末尾 `watchdog_update()` |
...
| 信号采集与调理 | 自实现 5 级可开关链 | 每级独立使能+参数可调，**所有分母参数必须强钳位** |
...
-   **看门狗强制。** 主循环末尾必须包含 `watchdog_update()`。
-   **USB 状态保持。** 在保存配置等耗时操作中，必须通过技术手段或上位机协议设计，防止 USB 主机端因超时将设备强制下线。
...
│  │(LittleFS封装) │  │(TinyUSB读写封装)│  │   (NeoPixel驱动)       │ │
│  └──────────────┘  └────────────────┘  └────────────────────────┘ │
│  ┌──────────────┐  ┌────────────────┐  ┌────────────────────────┐ │
...
| 模块 | 文件 | 职责边界 | 复用性 |
| :--- | :--- | :--- | :--- |
| adc_sampler | adc_sampler.h/c | 纯 ADC 单次读取（200Hz 由主循环调度） | 🟢 通用 |
| signal_conditioner | signal_conditioner.h/c | 5 级可开关信号调理链（含防除零钳位） | 🟢 通用 |
...
| flash_store | flash_store.h/c | LittleFS 文件读写抽象（挂载/读/写/删） | 🟢 通用 |
...
```
物理推杆 → RC硬件滤波 → RP2040 ADC → [adc_sampler 200Hz] → [signal_conditioner 5级可开关链] → MIDI CC 输出
```

### 6.2 `adc_sampler` 规格（200Hz 单次采样）

| 参数 | 值 | 说明 |
| :--- | :--- | :--- |
| 触发频率 | 200Hz (5ms/次) | 主循环 1ms 计数器驱动，每 5 次循环触发 1 次完整过采样 |
| 单次过采样 | 17 次连续读取 × 4 路 | 每次触发耗时 ~140µs，占 5ms 时间片约 2.8% |
| 平均方式 | 算术平均（整数除法） | 后 16 个 uint16_t 求和 ÷ 16 |
...
// 执行 17 次连续过采样并返回平均值，由主循环以 200Hz 频率调度
uint16_t adc_sampler_read(uint8_t pin, uint8_t count);
#endif
```

### 6.3 `signal_conditioner` 规格（5 级可开关链 + 防御加固）
...
**所有涉及分母的参数，在运算前必须进行非零和边界强钳位。** 这是防止上位机传入非法值导致除零死机或内存越界的硬性要求。
...
int8_t signal_conditioner_process(signal_conditioner_t *sc, uint16_t raw) {
    ...
    if (sc->calibration_enable) {
        ...
        if (range <= 0) return -1; // 🔥 v3.1: 防除零，直接返回无效值
    ...
    if (sc->change_threshold_enable) {
        ...
        if (thr < 0) thr = 0;       // 🔥 v3.1 钳位
        if (thr > 127) thr = 127;   // 🔥 v3.1 钳位
...
| save_config | `{"cmd":"save_config",...}` | `{"cmd":"save_config_ack","status":"ok"}` | 唯一写 Flash 的配置指令 |
| factory_reset | `{"cmd":"factory_reset"}` | `{"cmd":"factory_reset_ack","status":"ok"}` | 恢复出厂（写 Flash） |
...
> ⚠️ `flash_store_write_file()` 及其直接调用的底层函数**必须标记 `__not_in_flash_func()`**。这是 v3.1 中唯一需要该宏的地方。
...
void loop() {
    ...
    // 🔥 v3.1: 200Hz ADC 采样 (每 5ms 采 1 次)
    adc_counter++;
    if (adc_counter >= 5) {
        adc_counter = 0;

        // 1. ADC 过采样（17次×4路，~140µs / 5ms ≈ 2.8%）
...
| # | 现象 | 触发概率 | 用户影响 | 应对 | 备注 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | 保存配置时旋钮短暂失灵 (~400ms) | 极低 | 无感知 | 不处理 | Flash 写入期间主循环暂停 |
| 2 | 保存配置后 PC 端可能短暂断开 USB | 中 | 需等待自动重连 | 上位机 save_config 后等待 500ms 并自动重连 | 🔥 v3.1: 已通过 tud_task() 缓解，但无法完全避免 |
...
| 模块 | 开发指令重点 | 单测策略 |
| :--- | :--- | :--- |
| adc_sampler | 单次读取，无过采样 | PC Mock `analogRead` |
| signal_conditioner | 5 级可开关链 + **各级边界钳位测试** | PC 序列验证：非法参数注入、除零测试、极端输入 |
| flash_store | LittleFS 挂载与文件读写 | 烧录验证 |
...
### 上下文
本项目是 Lyre USB MIDI 控制器固件（RP2040 单核架构，v3.1 量产加固版）。
...
8. 🔥 信号链相关模块：所有分母参数必须在运算前进行非零和边界强钳位。
9. 🔥 主循环相关模块：末尾必须包含 watchdog_update()。

### 验收标准
1. [功能验收项]
2. [边界条件测试项]
3. 🔥 [防御钳位/WDT/USB保活 专项测试项]
```
...
| **v3.0→v3.1** | **ADC 阻塞式过采样致时间片抖动** | **改为 200Hz 单次采样（5ms/次）** | **adc_sampler, main.ino** |
...
**文档版本 v3.1 量产加固版 — 2026-07-10**
经过 9 轮架构审计迭代，从双核收敛为单核，信号链改为可调试可开关设计，所有过度设计已清除。v3.1 补全了 ADC 采样策略、信号链防御、USB 保活、看门狗兜底和量产可观测性五大生产级补丁。本文档为自包含完整规格书，可直接交付 AI 程序员进行逐模块代码生成。
